...
pub async fn state_logger(stats: std::sync::Arc<tokio::sync::RwLock<Stats>>, rpc_url: String, rpc_api_key: Option<String>) {
    ...
    loop {
        ...
        let time_to_catch_the_tip_duration = if block_processing_speed > 0.0 {
            if let Ok(block_height) = crate::configs::final_block_height(&rpc_url, &rpc_api_key).await {
